...
export default function ProjectView() {
  ...
  const connectionColor = isConnected ? 'bg-green-500' : isPolling ? 'bg-yellow-500' : 'bg-red-500';
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2 text-sm">
          <span className={`inline-block w-2.5 h-2.5 rounded-full ${connectionColor}`} />
          ...
          (viewMode === 'board' ? (
            ...
            <>
              {milestonesLoading ? (
                ...
                <MilestoneTimeline milestones={milestones} isArchitect={false} />
              )}
            </>
          ))}
      </div>
