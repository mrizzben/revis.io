...
async def test_admin_can_create_project(client, admin_headers):
    resp = await client.post("/api/projects", json={"name": "Admin Project"}, headers=admin_headers)
    assert resp.status_code == 201, resp.text
    assert resp.json()["owner_id"] is not None


async def test_admin_sees_all_projects(
    client, admin_headers, project, second_project, test_architect
):
...
async def test_collaborator_can_update_but_not_archive(client, collaborator_headers, project):
...
async def test_collaborator_cannot_enable_client_access(client, collaborator_headers, project):
    resp = await client.post(
        f"/api/projects/{project.id}/client-access",
        json={"password": "client-pass-123"},
        headers=collaborator_headers,
...
async def test_anonymous_session_cannot_access_internal_routes(client, auth_headers, project):
    token = (
        await client.post(
            f"/api/projects/{project.id}/client-access",
            json={"password": "client-pass-123"},
...
async def test_disable_client_access_revokes_sessions(client, auth_headers, project):
    ...
    assert (
        await client.get(f"/api/projects/{project.id}", headers=guest_headers)
    ).status_code == 200
